...
The employer has an owner org but can be shared with other orgs via access grants. The owner org can delegate access to an employer to another org (e.g. a group delegates payroll management of a subsidiary to an external accounting firm).
...
> **Note**: This is conceptually similar to how accounting platforms (e.g. QuickBooks Online Accountant) separate firm access from client data, but our implementation is different — employers are not separate subscriptions or isolated instances. They are entities within our system with access controlled via the `employer_access` table.
